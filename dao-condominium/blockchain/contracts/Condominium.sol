...
contract Condominium is ICondominium {
    ...
    function removeResident(address resident) external onlyManager {
        ...
        if(index == 0) {
            require(residents[index].wallet == resident, "This resident does not exists");
        }
    ...
    // COUNSELOR FUNCTIONS
    function _isCounselor(address resident) public view returns (bool) {
    ...
    function _getTopic(string memory title) private view returns (Lib.Topic memory) {
        ...
        if(index < topics.length){
            ...
            if(index > 0 || keccak256(bytes(result.title)) == topicId) return result;
        }
        return Lib.Topic({
            title: "",
            description: "",
